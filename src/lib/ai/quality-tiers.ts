...
- 20-200 parts. Focus on correct proportions and clean geometry.
...
- 40-500 parts. Every surface needs thought — no plain boxes.
...
- USE FOR LOOPS for repeated elements (fence posts, windows, floor tiles).`,
  },
...
- 60-2000 parts. USE FOR LOOPS for repeated elements (windows, fence posts, tiles, lamps, trees, building rows). Real Roblox games have 14K-25K parts. A single lighthouse = 430 parts. Your builds must be professional.
- For large builds (maps, towns, worlds): use table-driven generation. Define configs in a table, loop through them. 12 buildings from one loop = 200+ parts with zero repetitive code.
- PROCEDURAL TERRAIN: Use workspace.Terrain:FillBlock/FillBall in loops for varied ground. Generate hills with math.sin/cos height variation. Never hand-place terrain.
- BUILD IN PHASES: For 500+ part builds, organize code into clearly labeled sections (-- PHASE 1: Terrain, -- PHASE 2: Buildings, etc.) so the output stays structured.
...
- ORGANIC VARIATION: vary window sizes by 0.5 studs, offset parts by 0.1-0.2 studs for natural imperfection.`,
  ...
  master: {
    ...
    requiresCinematicLighting: true,
    promptModifier: `QUALITY TIER: MASTER — Showcase, award-winning quality. Target 100-5000 parts.
- USE FOR LOOPS AND TABLES FOR EVERYTHING. Never hand-place more than 5 similar objects. Define building configs, prop configs, tree configs as tables and loop through them.
- PROCEDURAL GENERATION: Use math functions for organic layouts. Trees placed with math.random offsets. Hills generated with nested FillBall loops. Streets laid out with parametric spacing.
- BUILD IN PHASES for large builds:
  Phase 1: Terrain base (FillBlock ground + FillBall hills + Water)
  Phase 2: Road/path network (loop-generated)
  Phase 3: Building shells (table-driven, 10-30 buildings from one config table)
  Phase 4: Building details (windows, doors, signs — inner loops)
  Phase 5: Props and decoration (lamp posts, benches, trees — scatter loops)
  Phase 6: Lighting and effects (PointLights in every building, ambient particles)
  Phase 7: Scripts and interactivity (doors, NPCs, shops)
...
- Architectural detail: crown molding, chair rail, wainscoting, decorative arches, corbels.`,
  ...
  legendary: {
    ...
    requiresScriptedElements: true,
    requiresCinematicLighting: true,
    promptModifier: `QUALITY TIER: LEGENDARY — God tier. Full game worlds. 200-50000 parts.
- MEGA-SCALE GENERATION: This tier builds ENTIRE GAME WORLDS. Use aggressive procedural generation:
  * Define ALL buildings/props/trees as config tables with {name, size, colors, material, position offset}
  * Loop through configs to generate hundreds of objects from compact code
  * Use nested for-loops: outer loop for zones/streets, inner loop for buildings/props per zone
  * Parametric placement: math.sin/cos for curved streets, math.random for organic scatter
  * Factory functions: createBuilding(config), createTree(pos, size), createLampPost(pos) — call them in loops
- BUILD IN PHASES (label each section clearly):
  Phase 1: World terrain (FillBlock zones + FillBall mountains + Water bodies — loop-generated)
  Phase 2: Road/path network (parametric curves, intersections, sidewalks)
  Phase 3: Zone generation (3-8 themed zones, each with distinct palette and building style)
  Phase 4: Building generation (table-driven, 20-100 buildings per zone from config loops)
  Phase 5: Interior furnishing (per-room templates applied via loops)
  Phase 6: Prop scatter (trees, lamps, benches, signs — random offset placement in loops)
  Phase 7: NPC and interactive systems (shops, quest givers, doors, elevators)
  Phase 8: Lighting stack (per-building lights, ambient particles, cinematic post-processing)
  Phase 9: Game scripts (economy, combat, quests, data saving, UI)
  Phase 10: Polish (sounds, beams, highlights, weather effects)
...
- TERRAIN EVERYWHERE: FillBlock ground, FillBall hills, Water ponds, Sand beaches. NO Part-based terrain. Use loops with math functions for natural terrain variation.
