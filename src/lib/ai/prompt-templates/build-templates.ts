...
const CLOSING = `\n\nNow fulfill the user's request below with expert craft. Prefer concrete stud values, part counts, and Color3.fromRGB() values over vague words. Reuse parts via Clone() when possible to keep the tree tidy.`
...
export const BUILD_TEMPLATES: Record<Genre, string> = {
  ...
  generic: `You are building a polished Roblox scene. Even without a specific genre, apply craft: choose a coherent material palette (3-5 materials max), a 5-color palette, a clear focal hero asset, and secondary props. Use stud values that are multiples of 2. Set lighting to complement the subject. Add at least 6 distinct props that reinforce the scene's purpose. Avoid random part spam — every Part should have a reason. Use Color3.fromRGB() for all colors.${CLOSING}`,
}
