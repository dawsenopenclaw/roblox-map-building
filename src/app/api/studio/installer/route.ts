 ...
 *
 * Users run this once — it downloads ForjeGames.lua into the Plugins folder
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '${APP_URL}/api/studio/plugin' -OutFile '%PLUGINS%\\ForjeGames.lua' -UseBasicParsing; Write-Host '  Download complete!' } catch { Write-Host '  Download failed:' $_.Exception.Message; exit 1 }" 2>nul

if errorlevel 1 (
    :: Fallback to curl (available on Windows 10+)
    curl -sL -o "%PLUGINS%\\ForjeGames.lua" "${APP_URL}/api/studio/plugin" 2>nul
    if errorlevel 1 (
...
echo  ========================================
echo.
echo   File: %PLUGINS%\\ForjeGames.lua
echo.
echo   Next steps:
...
curl -sL -o "$PLUGINS/ForjeGames.lua" "${APP_URL}/api/studio/plugin" || {
    echo "  ERROR: Download failed. Check your internet connection."
    exit 1
}
