...
export default function SignUpClient() {
  ...
  const handleRobloxSignUp = async () => {
    ...
    try {
      ...
      console.error('Roblox sign-up failed:', e)
      setLoading(null)
    }
  }
  ...
  const handleOAuthSignUp = async (strategy: 'oauth_google' | 'oauth_apple') => {
    ...
    try {
      ...
      console.error(`${strategy} sign-up failed:`, e)
      setLoading(null)
    }
  }
  ...
  return (
    <div className="w-full">
      ...
      {/* Primary: Roblox Sign Up */}
      <button
        onClick={handleRobloxSignUp}
        disabled={!isLoaded || loading !== null}
        className="w-full flex items-center justify-center gap-3 h-12 rounded-lg bg-[#00A2FF] hover:bg-[#0090E0] text-white font-semibold text-base transition-colors disabled:opacity-50 disabled:cursor-not-allowed mb-3"
      >
        {loading === 'roblox' ? (
        ...
        {loading === 'roblox' ? 'Connecting to Roblox...' : 'Sign up with Roblox'}
      </button>

      <p className="text-xs text-center text-zinc-500 mb-4">
        We only see your Roblox username and avatar. We never see your password.
      </p>
      ...
      <div className="relative my-5">
        ...
        <div className="relative flex justify-center">
          <span className="px-3 text-xs text-zinc-600" style={{ background: '#0a0d14' }}>or continue with</span>
        </div>
      </div>
      ...
      <div className="mt-6 pt-5 border-t border-white/[0.06]">
        <p className="text-sm text-center text-zinc-500">
          ...
          <Link href="/sign-in" className="font-semibold text-[#D4AF37] hover:text-[#E6A619] transition-colors">
            Sign in
          </Link>
        </p>
      </div>
