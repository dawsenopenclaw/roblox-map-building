...
function AssetThumbnail({ asset }: { asset: RobloxAsset }) {
  ...
  return (
    <div className="relative mb-2 h-16 w-full overflow-hidden rounded-md bg-white/5">
      <Image src={src} alt={asset.name} fill className="object-cover" onError={() => setImgError(true)} unoptimized />
    </div>
  )
}
    ...
    try {
      if (genType === 'mesh') {
        ...
        const data = (await res.json()) as {
          meshUrl?: string | null; fbxUrl?: string | null; thumbnailUrl?: string | null
          videoUrl?: string | null; polygonCount?: number | null
          textures?: { albedo: string; normal: string; roughness: string } | null
          luauCode?: string | null; costEstimateUsd?: number; status: string; taskId?: string | null
        }
        ...
        setGeneratedAssets((prev) => prev.map((a) => a.id !== id ? a : {
          ...a, status: nextStatus, meshUrl: data.meshUrl, fbxUrl: data.fbxUrl,
          thumbnailUrl: data.thumbnailUrl, videoUrl: data.videoUrl, polygonCount: data.polygonCount,
          textures: data.textures, luauCode: data.luauCode, costEstimateUsd: data.costEstimateUsd, taskId: data.taskId,
        }))
      } else {
        ...
        if (!res.ok) throw new Error(`API error ${res.status}`)
        const data = (await res.json()) as {
          textureUrl?: string | null; normalUrl?: string | null; roughnessUrl?: string | null
          resolution?: string; luauCode?: string | null; costEstimateUsd?: number; status: string
        }
        const nextStatus: GeneratedAsset['status'] = data.status === 'demo' ? 'demo' : data.status === 'complete' ? 'complete' : 'loading'
        setGeneratedAssets((prev) => prev.map((a) => a.id !== id ? a : {
          ...a, status: nextStatus, textureUrl: data.textureUrl, normalUrl: data.normalUrl,
          roughnessUrl: data.roughnessUrl, thumbnailUrl: data.textureUrl,
          textures: data.textureUrl && data.normalUrl && data.roughnessUrl
            ? { albedo: data.textureUrl, normal: data.normalUrl, roughness: data.roughnessUrl } : null,
          resolution: data.resolution, luauCode: data.luauCode, costEstimateUsd: data.costEstimateUsd,
        }))
      }
      setGenPrompt('')
    } catch (err) {
      ...
      <div className="flex shrink-0 border-b border-white/10">
        {(['marketplace', 'generate'] as const).map((tab) => (
          <button
            ...
            className={['flex-1 py-2.5 text-[11px] font-semibold transition-colors', activeTab === tab ? 'border-b-2 border-[#FFB81C] text-[#FFB81C]' : 'text-gray-500 hover:text-gray-300'].join(' ')}
          >
          ...
          <div className="flex flex-wrap gap-1">
            {ASSET_CATEGORIES.map((cat) => (
              <button
                ...
                className={['rounded-full px-2.5 py-1 text-[10px] font-semibold transition-colors', activeCategory === cat ? 'bg-[#FFB81C] text-black' : 'border border-white/10 bg-white/5 text-gray-300 hover:bg-white/10 hover:text-gray-200'].join(' ')}
              >{cat}</button>
            ))}
          </div>
        </div>
        <div className="flex-1 overflow-y-auto px-3 pb-3">
          {error ? (
            <div className="py-6 text-center">
              ...
              <button onClick={() => setRetryKey((k) => k + 1)} className="text-[10px] text-[#FFB81C] hover:underline">Retry</button>
            </div>
          ) : loading ? (
            ...
            <p className="py-8 text-center text-xs text-gray-600">{debouncedQuery ? 'No results found' : 'Loading marketplace...'}</p>
          ) : (
            <div className="grid grid-cols-2 gap-2">
              {results.map((asset) => {
                ...
                return (
                  <div key={asset.id} className="flex flex-col rounded-lg border border-white/8 bg-white/[0.03] p-2.5 transition-colors hover:border-white/[0.18]">
                    ...
                    <p className="line-clamp-2 flex-1 text-[11px] font-medium leading-tight text-gray-300" title={asset.name}>{asset.name}</p>
                    <p className="mt-0.5 truncate text-[9px] text-gray-600">{asset.creatorName}</p>
                    <div className="mt-1.5 flex items-center justify-between">
                      <span className={['text-[10px] font-bold', free ? 'text-emerald-400' : 'text-[#FFB81C]'].join(' ')}>{label}</span>
                      <button
    ...
    <div className="flex h-full flex-col space-y-3 p-3">
      ...
      {generated && (
        <div className="overflow-hidden rounded-lg border border-white/10 bg-white/[0.03]">
          <div className="flex h-20 w-full items-center justify-center text-3xl" style={{ background: `linear-gradient(135deg, ${generated.gradient[0]}, ${generated.gradient[1]})` }}>
            &#10024;
          </div>
          <div className="p-2.5">
            ...
            <button className="mt-2 w-full rounded bg-[#FFB81C]/15 py-1 text-[9px] font-bold text-[#FFB81C] transition-colors hover:bg-[#FFB81C]/30">Use in Game</button>
          </div>
        </div>
      )}
    </div>
        ...
        <div className="flex flex-wrap gap-1">
          {COMMUNITY_CATEGORIES.map((cat) => (
            <button key={cat} onClick={() => setActiveCat(cat)}
              className={['rounded-full px-2 py-0.5 text-[9px] font-semibold transition-colors', activeCat === cat ? 'bg-[#FFB81C] text-black' : 'border border-white/10 bg-white/5 text-gray-400 hover:bg-white/10 hover:text-gray-200'].join(' ')}>
              {cat}
            </button>
          ))}
        </div>
          ...
          <div className="flex items-center gap-1">
            ...
            {SORT_OPTIONS.map((s) => (
              <button key={s} onClick={() => setSort(s)}
                className={['rounded px-1.5 py-0.5 text-[9px] font-semibold transition-colors', sort === s ? 'bg-white/15 text-white' : 'text-gray-500 hover:text-gray-300'].join(' ')}>
                {s}
              </button>
            ))}
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-3">
        {filtered.length === 0 ? (
          <div className="py-10 text-center space-y-2">
            ...
            <button onClick={() => { setQuery(''); setActiveCat('All') }} className="text-[10px] text-[#FFB81C]/70 hover:text-[#FFB81C] transition-colors">Clear filters</button>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-2">
            {filtered.map((asset) => {
              ...
              return (
                <div key={asset.id} className="overflow-hidden rounded-lg border border-white/8 bg-white/[0.02] transition-all hover:border-white/[0.18]">
                  <div className="relative flex h-14 w-full items-end justify-between p-1.5" style={{ background: `linear-gradient(135deg, ${asset.gradientFrom}, ${asset.gradientTo})` }}>
                    <span className="rounded px-1 py-0.5 text-[7px] font-bold text-white/90" style={{ background: STYLE_COLORS[asset.style] + 'CC' }}>{asset.style}</span>
                    <span className="rounded bg-black/50 px-1 py-0.5 text-[7px] font-mono text-white/80">{formatPoly(asset.polyCount)}p</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-1 px-2 pb-2">
                    <button onClick={() => handleInsert(asset)} disabled={isIns}
                      className={['rounded py-1 text-[8px] font-bold transition-all', isIns ? 'cursor-default bg-emerald-500/20 text-emerald-400' : 'bg-[#FFB81C]/15 text-[#FFB81C] hover:bg-[#FFB81C]/25 active:scale-95'].join(' ')}>
...
// ─── Main AssetsPanel export ──────────────────────────────────────────────────

type AssetTab = 'roblox' | 'generate' | 'community'

export const AssetsPanel = React.memo(function AssetsPanel({ onInsertAsset }: { onInsertAsset?: (luau: string, name: string) => void }) {
  ...
  return (
    <div className="flex h-full flex-col">
        ...
        <div className="flex gap-0.5 rounded-lg bg-white/5 p-0.5">
          {tabs.map(({ id, label }) => (
            <button
              ...
              className={['flex-1 rounded-md py-1.5 text-[10px] font-semibold transition-all', activeTab === id ? 'bg-[#FFB81C] text-black shadow-sm' : 'text-gray-400 hover:text-gray-200'].join(' ')}
            >{label}</button>
          ))}
        </div>
      </div>
      <div className="flex-1 overflow-hidden">
        ...
        {activeTab === 'generate'  && <GenerateTabPanel />}
