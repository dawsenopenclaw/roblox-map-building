  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="h-7 w-36 rounded-xl shimmer mb-2" />
          <div className="h-4 w-28 rounded-md shimmer" />
        </div>
        {/* Progress percentage */}
        <div className="text-right">
          <div className="h-9 w-16 rounded-xl shimmer mb-1" />
        ...
        <div key={cat} className="mb-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="bg-[#0D1231] border border-white/10 rounded-xl p-3 text-center"
              >
                {/* Icon placeholder */}
