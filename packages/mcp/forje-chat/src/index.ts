...
if (!API_KEY) {
  process.stderr.write(
    '[forje-mcp] WARNING: FORJE_API_KEY not set. Get one at forjegames.com/settings\n',
  )
}
...
async function forjePost(path: string, body: Record<string, unknown>): Promise<unknown> {
...
async function forjeGet(path: string): Promise<unknown> {
  const url = `${API_BASE}${path}`
  const res = await fetch(url, {
    headers: {
      Authorization: `Bearer ${API_KEY}`,
...
server.tool(
  'forje_build',
  'Send a building prompt to ForjeGames AI. The AI generates Roblox Luau code and sends it to your connected Studio session. Use this for any custom build: houses, vehicles, terrain, NPCs, game systems, etc.',
  {
    prompt: z.string().describe('What to build in Roblox Studio (e.g. "a medieval castle with towers and a moat")'),
    style: z.string().optional().describe('Art style hint: realistic, cartoon, low-poly, sci-fi, fantasy, horror'),
    quality: z.enum(['quick', 'normal', 'detailed']).optional().describe('Build quality — quick (10-20 parts), normal (30-50 parts), detailed (50-100+ parts)'),
  },
  async ({ prompt, style, quality }) => {
    try {
      ...
      const message = (result.message as string) || 'Build sent to Studio'
      ...
      return {
        content: [{ type: 'text' as const, text: `Build failed: ${(err as Error).message}` }],
        isError: true,
      }
    }
  },
...
server.tool(
  'forje_script',
  'Generate a Roblox Luau script and send it to Studio. Use for game logic: NPCs, combat, inventory, UI, etc.',
  {
    prompt: z.string().describe('What the script should do (e.g. "NPC that follows the player and sells items")'),
    script_type: z
  ...
  async ({ prompt, script_type }) => {
    try {
      ...
      const message = (result.message as string) || 'Script sent to Studio'
      const tokensUsed = (result.tokensUsed as number) || 0
...
server.tool(
  'forje_template',
  'Build an instant template in Roblox Studio. These are pre-made, high-quality builds that appear instantly.',
  {
    template: z
      .string()
      .describe(
        `Template ID. Options: ${INSTANT_TEMPLATES.map((t) => t.id).join(', ')}`,
      ),
  },
  async ({ template }) => {
    try {
      ...
      const message = (result.message as string) || `${tmpl.name} built!`
      ...
      return {
        content: [{ type: 'text' as const, text: `Template failed: ${(err as Error).message}` }],
        isError: true,
      }
    }
  },
...
server.tool(
  'forje_status',
  'Check your ForjeGames account status — Studio connection, token balance, subscription tier.',
  {},
  async () => {
    try {
      const result = (await forjeGet('/api/mcp/status')) as Record<string, unknown>
      ...
      return {
        content: [
          { type: 'text' as const, text: `Status check failed: ${(err as Error).message}` },
        ],
        isError: true,
      }
    }
...
server.tool(
  ...
  'List all available instant build templates with descriptions.',
  {},
  async () => {
    const lines = INSTANT_TEMPLATES.map(
