...
-- ============================================================
-- Workspace part counting — counts actual BaseParts for feedback
-- Capped at 100K descendants to prevent freezing on massive maps.
-- ============================================================
local _partCountCache = { value = -1, time = 0 }
local PART_COUNT_CACHE_TTL = 0.5 -- seconds — prevents double-scan within same frame

local function countWorkspaceParts()
  -- Return cached value if fresh (avoids double-scan for before/after in same tick)
  local now = tick()
  if _partCountCache.value >= 0 and (now - _partCountCache.time) < PART_COUNT_CACHE_TTL then
    return _partCountCache.value
  end

  local count = 0
  pcall(function()
    local descendants = workspace:GetDescendants()
    local limit = math.min(#descendants, 100000)
    for i = 1, limit do
      if descendants[i]:IsA("BasePart") then
        count = count + 1
      end
    end
    if #descendants > 100000 then
      warn("[ForjeGames] Workspace has " .. #descendants .. " descendants — part count is approximate")
    end
  end)

  _partCountCache.value = count
  _partCountCache.time = now
  return count
end

-- Invalidate cache after a build so the "after" count is fresh
local function invalidatePartCountCache()
  _partCountCache.value = -1
  _partCountCache.time = 0
end

-- ============================================================
-- Build result reporter — POSTs real execution data to server
-- Runs non-blocking via task.spawn so it never delays the plugin
-- ============================================================
local function reportBuildResult(commandId, success, partsBefore, partsAfter, errors, executionTimeMs, originalCode)
  task.spawn(function()
    pcall(function()
      local resultData = {
        ...
        commandId = commandId,
        delta = {
          partCountBefore = partsBefore,
          partCountAfter = partsAfter,
          errors = errors or {},
          timestamp = os.time(),
        },
        originalCode = originalCode and tostring(originalCode):sub(1, 2000) or nil,
      }
      httpPost("/api/studio/build-result", resultData)
    end)
  end)
end
...
local function executeStructuredCommands(commands, commandId)
  ...
  -- Snapshot workspace BEFORE execution for real delta tracking
...
task.defer(function()
  script:Destroy()
    ...
    if cmdOk then
      ...
      local errMsg = tostring(cmd.type) .. ": " .. tostring(cmdErr)
      table.insert(buildErrors, errMsg)
      warn("[ForjeGames] Structured command error (" .. errMsg .. ")")
    end
  end
  ...
  -- Snapshot workspace AFTER execution — real delta, not command counts
  invalidatePartCountCache()
  local partsAfter = countWorkspaceParts()
  local executionTimeMs = math.floor((tick() - startTime) * 1000)
  local actualPartsCreated = partsAfter - partsBefore

  print("[ForjeGames] Build result: " .. actualPartsCreated .. " parts created (" .. partsAfter .. " total), " .. executionTimeMs .. "ms, " .. #buildErrors .. " errors")
  ...
  if commandId then
    Sync.queueChange("command_result", {
      ...
      partsCreated = actualPartsCreated,
    ...
    -- Also POST directly to build-result for the error learning system
    if #buildErrors > 0 then
      reportBuildResult(commandId, failCount == 0, partsBefore, partsAfter, buildErrors, executionTimeMs, nil)
    end
  end
  ...
  if data.code and type(data.code) == "string" then
    ...
    elseif hasComplexCode then
      ...
      if allowLoadstring then
        ...
        -- Snapshot workspace BEFORE loadstring execution
        local lsPartsBefore = countWorkspaceParts()
        ...
        -- Snapshot workspace AFTER — get REAL part count delta
        invalidatePartCountCache()
        local lsPartsAfter = countWorkspaceParts()
        local lsExecTimeMs = math.floor((tick() - lsStartTime) * 1000)
        local lsActualParts = lsPartsAfter - lsPartsBefore

        if loadOk then
          print("[ForjeGames] Loadstring result: " .. lsActualParts .. " parts created (" .. lsPartsAfter .. " total), " .. lsExecTimeMs .. "ms")
          playCompletionSound()
          if commandId then
            Sync.queueChange("command_result", {
              commandId = commandId, success = true,
              partsCreated = lsActualParts, partsFailed = 0, totalCommands = 1,
              method = "loadstring",
              executionTimeMs = lsExecTimeMs,
              partCountBefore = lsPartsBefore,
              partCountAfter = lsPartsAfter,
            })
          end
        else
          ...
          if commandId then
            local errStr = tostring(loadErr)
            Sync.queueChange("command_result", {
              commandId = commandId, success = false,
              partsCreated = lsActualParts, partsFailed = 1, totalCommands = 1,
              error = errStr, method = "loadstring",
              executionTimeMs = lsExecTimeMs,
              partCountBefore = lsPartsBefore,
              partCountAfter = lsPartsAfter,
              errors = { errStr },
            })
            -- Feed error into learning system
            reportBuildResult(commandId, false, lsPartsBefore, lsPartsAfter, { errStr }, lsExecTimeMs, code)
          end
        end
      else
